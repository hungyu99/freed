...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		freedMessage, err := client.Post(&protowire.FreedMessage{Payload: &protowire.FreedMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := freedMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	freedMessage := &protowire.FreedMessage{}
	err := protojson.Unmarshal([]byte(response), freedMessage)
	if err != nil {
	...
	return marshalOptions.Format(freedMessage)
}
