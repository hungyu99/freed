...
Clients are expected to build RequestMessages and wrap them in FreedMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a FreedMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a FreedMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network freed is currently running against.
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether freed thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when freed is not synced. That is because when freed isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known freed addresses in the
...
currently connected to this freed.
...
| isOutbound | [bool](#bool) |  | Whether this freed initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this freed |
...
AddPeerRequestMessage adds a peer to freed&#39;s outgoing connection list.
This will, in most cases, result in freed connecting to said peer.
...
parent chain from some startHash to this freed&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this freed.
...
ShutDownRequestMessage shuts down this freed.
...
This call is only available when this freed was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given freed addresses
