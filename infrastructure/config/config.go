...
const (
	defaultConfigFilename      = "freed.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "freed.log"
	defaultErrLogFilename      = "freed_err.log"
	...
	sampleConfigFilename    = "sample-freed.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for freed.
	DefaultAppDir = util.AppDir("freed", false)
...
// Flags defines the configuration options for freed.
//
...
type Flags struct {
	...
	Listeners                       []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 22111, testnet: 16211)"`
	...
	RPCListeners                    []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 22110, testnet: 16210)"`
...
// Config defines the configuration options for freed.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default freed configuration
...
//
// The above results in freed functioning properly without any config settings
...
// createDefaultConfig copies the file sample-freed.conf to the given destination path,
